...
from PyQt5.QtWidgets import (
    ...
    QGridLayout, QVBoxLayout, QHBoxLayout, QProgressBar, QMessageBox, QSizePolicy, QScrollArea
)
...
def result_model_time_ms(res, fallback_ms=0.0):
    """Use Ultralytics' own timing to avoid counting UI, image display and first-call setup."""
    speed = getattr(res, "speed", None)
    if isinstance(speed, dict):
        total_ms = 0.0
        for key in ("preprocess", "inference", "postprocess"):
            try:
                total_ms += float(speed.get(key, 0.0) or 0.0)
            except (TypeError, ValueError):
                pass
        if total_ms > 0:
            return total_ms
    return float(fallback_ms or 0.0)


def predict_one_timed(model, source, **kwargs):
    t0 = time.perf_counter()
    res = model.predict(source, **kwargs)[0]
    wall_ms = (time.perf_counter() - t0) * 1000.0
    model_ms = result_model_time_ms(res, wall_ms)
    return res, model_ms, wall_ms


def warmup_model(model, imgsz=640):
    dummy = np.zeros((imgsz, imgsz, 3), dtype=np.uint8)
    model.predict(
        dummy, device=INFER_DEVICE, imgsz=imgsz, conf=0.3, iou=0.5, verbose=False
    )
    ...
    def run(self):
        ...
        while not self._stop:
            ...
            if i % self.step != 0:   # 抽帧提速（CPU 推荐 step=2或3）
                ...
                continue
            i += 1
            res, used, _ = predict_one_timed(
                self.model, frame, device=INFER_DEVICE, imgsz=self.imgsz,
...
class BatchWorker(QThread):
    ...
    finished = pyqtSignal(str, int, int, bool)  # output_dir, processed, total, stopped
    error = pyqtSignal(str)

    def __init__(self, model: YOLO, input_dir: str, output_dir: str, model_name: str,
                 imgsz=640, conf=0.3, iou=0.5):
        super().__init__()
        self.model = model
    ...
    def run(self):
        ...
        processed = 0
        stopped = False
        result_dir = os.path.join(self.output_dir, "detected_images")
        os.makedirs(result_dir, exist_ok=True)
        report_path = os.path.join(self.output_dir, "detection_report.csv")

        try:
            with open(report_path, "w", newline="", encoding="utf-8-sig") as f:
                writer = csv.writer(f)
                writer.writerow([
                    "image_index", "image_name", "image_path", "result_image", "model_name",
                    "inference_ms", "object_count", "class_name", "confidence",
                    "x1", "y1", "x2", "y2"
                ])
                ...
                for idx, image_path in enumerate(images, start=1):
                    ...
                    try:
                        res, used, _ = predict_one_timed(
                            self.model, image_path, device=INFER_DEVICE, imgsz=self.imgsz,
                            conf=self.conf, iou=self.iou, verbose=False
                        )
                        vis = res.plot()
                        stem = os.path.splitext(image_name)[0]
                        result_image = os.path.join(result_dir, f"{idx:04d}_{stem}_det.jpg")
                        cv2.imwrite(result_image, vis)

                        info = result_to_info(res)
                        info.update({"frame_index": idx, "source_name": image_path})
                        detections = info["detections"]
                        if detections:
                            for det in detections:
                                writer.writerow([
                                    idx, image_name, image_path, result_image, self.model_name,
                                    f"{used:.3f}", len(detections), det["class_name"],
                                    f"{det['confidence']:.6f}", det["x1"], det["y1"], det["x2"], det["y2"]
                                ])
                        else:
                            writer.writerow([
                                idx, image_name, image_path, result_image, self.model_name,
                        ...
                        processed += 1
                        self.frame_ready.emit(vis, used, info, res)
                        self.progress_ready.emit(processed, total, image_name)
                    except Exception as e:
                        self.error.emit(f"{image_name}: {e}")
                        writer.writerow([
                            idx, image_name, image_path, "", self.model_name,
                            "", 0, "ERROR", str(e), "", "", "", ""
                        ])
                        processed += 1
                        self.progress_ready.emit(processed, total, image_name)
        ...
        self.finished.emit(self.output_dir, processed, total, stopped)
...
class Main(QMainWindow):
    def __init__(self):
        ...
        self.btnBatch = QPushButton("📁 批量图片检测")
        self.btnStopBatch = QPushButton("⏸ 停止批量")
        ...
        self.btnBatch.setObjectName("btnPrimary")
        self.btnStopBatch.setObjectName("btnDanger")
        
        # 设置按钮高度
        BUTTON_HEIGHT = 56
        for btn in [
            self.btnCamera, self.btnOpenImg, self.btnOpenVid, self.btnSave,
            self.btnStop, self.btnClear, self.btnBatch, self.btnStopBatch
        ]:
        ...
        btnGridLayout.addWidget(self.btnBatch, 2, 0, 1, 2)
        btnGridLayout.addWidget(self.btnStopBatch, 2, 2)

        self.batchProgress = QProgressBar()
        self.batchProgress.setRange(0, 100)
        self.batchProgress.setValue(0)
        self.batchProgress.setFormat("批量检测进度：0/0")
        self.batchProgress.setMinimumHeight(18)
        btnGridLayout.addWidget(self.batchProgress, 3, 0, 1, 3)
        ...
        btnContainer.setFixedHeight(230)
        ...
        compareTitle = QLabel("⚖️ 模型性能在线对比")
        ...
        compareLayout.addWidget(QLabel("对比模型A"))
        self.cmbCompareA = QComboBox()
        compareLayout.addWidget(self.cmbCompareA)

        compareLayout.addWidget(QLabel("对比模型B"))
        self.cmbCompareB = QComboBox()
        compareLayout.addWidget(self.cmbCompareB)
        for model_id in MODELS.keys():
            self.cmbCompareA.addItem(MODEL_SHORT_NAMES[model_id], model_id)
            self.cmbCompareB.addItem(MODEL_SHORT_NAMES[model_id], model_id)
        if self.cmbCompareB.count() > 1:
            self.cmbCompareB.setCurrentIndex(1)
        for combo in (self.cmbCompareA, self.cmbCompareB):
            combo.setStyleSheet(self.cmbModel.styleSheet())

        compareButtons = QVBoxLayout()
        compareButtons.setSpacing(6)
        self.btnCompare = QPushButton("⚖️ 模型对比")
        self.btnSaveCompare = QPushButton("保存对比结果")
        self.btnCompare.setObjectName("btnCompareAction")
        self.btnSaveCompare.setObjectName("btnCompareAction")
        for btn in (self.btnCompare, self.btnSaveCompare):
            btn.setFixedHeight(36)
            btn.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Fixed)
            btn.setCursor(Qt.PointingHandCursor)
        compareButtons.addWidget(self.btnCompare)
        compareButtons.addWidget(self.btnSaveCompare)
        compareLayout.addLayout(compareButtons)

        self.vCompareSummary = QLabel("暂无对比结果")
        self.vCompareSummary.setWordWrap(True)
        self.vCompareSummary.setStyleSheet("""
            color: #ffffff;
            font-size: 12px;
            line-height: 1.35;
            padding: 8px;
        ...
        """)
        compareLayout.addWidget(self.vCompareSummary)

        rbox.addWidget(compareFrame)
        ...
        mainInfoLayout.addWidget(lab("⏱️ 模型耗时"), 0, 0)
        self.vTime = val()
        ...
        statsGrid.addWidget(lab("平均模型耗时"), 0, 0)
        self.vAvgTime = val()
        statsGrid.addWidget(self.vAvgTime, 0, 1)

        statsGrid.addWidget(lab("理论FPS"), 1, 0)
        self.vFPS = val()
        ...
        self.labDetectionFrames = lab("有目标帧数")
        statsGrid.addWidget(self.labDetectionFrames, 3, 0)
        ...
        self.btnCompare.clicked.connect(self.on_model_compare)
        self.btnSaveCompare.clicked.connect(self.on_save_compare)
        self.btnBatch.clicked.connect(self.on_open_batch)
        self.btnStopBatch.clicked.connect(self.on_stop_batch)
        ...
        #btnCompareAction:hover {
            background: rgba(67, 56, 202, 0.96);
            border-color: rgba(167, 139, 250, 1.0);
        }
        #btnCompareAction:pressed {
    ...
    def get_model_by_id(self, model_id):
        ...
        try:
            self.statusBar().showMessage(f"⏳ 正在加载模型: {MODEL_DISPLAY_NAMES[model_id]}...")
            model = YOLO(model_path)
            self.statusBar().showMessage(f"⏳ 正在预热模型: {MODEL_DISPLAY_NAMES[model_id]}...")
            warmup_model(model)
            self.model_cache[model_id] = model
            self.statusBar().showMessage(f"🔥 {MODEL_DISPLAY_NAMES[model_id]} 加载成功！")
            return model
        except Exception as e:
    ...
    # ========== 模型对比 ==========
    def _compare_metrics_from_result(self, model_id, res, used_ms):
        info = result_to_info(res)
        detections = info["detections"]
        confs = [float(det["confidence"]) for det in detections]
        avg_conf = float(np.mean(confs)) if confs else 0.0
        max_conf = float(np.max(confs)) if confs else 0.0
        fps = 1000.0 / used_ms if used_ms > 0 else 0.0
        return {
            "model_id": model_id,
            "model_name": MODEL_SHORT_NAMES.get(model_id, model_id),
            "time_ms": float(used_ms),
            "fps": fps,
    ...
    def _resize_to_height(self, img, height):
        ...
        width = max(int(img.shape[1] * scale), 1)
        return cv2.resize(img, (width, height), interpolation=cv2.INTER_AREA)

    def _make_compare_canvas(self, vis_a, metrics_a, vis_b, metrics_b):
        height = max(vis_a.shape[0], vis_b.shape[0])
        left = self._resize_to_height(vis_a, height)
        right = self._resize_to_height(vis_b, height)
        title_h = 54
        separator = np.full((height + title_h, 6, 3), (59, 130, 246), dtype=np.uint8)
        left_canvas = cv2.copyMakeBorder(left, title_h, 0, 0, 0, cv2.BORDER_CONSTANT, value=(15, 23, 42))
        right_canvas = cv2.copyMakeBorder(right, title_h, 0, 0, 0, cv2.BORDER_CONSTANT, value=(15, 23, 42))
        cv2.putText(left_canvas, f"A: {metrics_a['model_id']}  {metrics_a['time_ms']:.1f}ms  {metrics_a['object_count']} objs",
                    (16, 34), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 2, cv2.LINE_AA)
        cv2.putText(right_canvas, f"B: {metrics_b['model_id']}  {metrics_b['time_ms']:.1f}ms  {metrics_b['object_count']} objs",
                    (16, 34), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 2, cv2.LINE_AA)
        return np.concatenate([left_canvas, separator, right_canvas], axis=1)

    def _format_compare_summary(self, metrics_a, metrics_b):
        def one_line(tag, metrics):
            return (
                f"{tag} {metrics['model_name']}\n"
                f"用时: {metrics['time_ms']:.1f} ms | FPS: {metrics['fps']:.2f}\n"
                f"目标数: {metrics['object_count']} | 平均置信度: {metrics['avg_conf']:.3f}\n"
                f"最高置信度: {metrics['max_conf']:.3f} | 主类别: {metrics['main_class']}\n"
                f"Params: {metrics['params']} | FLOPs: {metrics['flops']}"
            )
        return one_line("A", metrics_a) + "\n\n" + one_line("B", metrics_b)

    def on_model_compare(self):
        self.on_stop_video()
        image_path, _ = QFileDialog.getOpenFileName(self, "选择对比图片", "", "Images (*.jpg *.jpeg *.png *.bmp)")
        if not image_path:
            return

        model_a_id = self.cmbCompareA.currentData()
        model_b_id = self.cmbCompareB.currentData()
        model_a = self.get_model_by_id(model_a_id)
        model_b = self.get_model_by_id(model_b_id)
        if model_a is None or model_b is None:
            return

        try:
            self.statusBar().showMessage("⚖️ 正在进行模型对比推理...")
            res_a, used_a, _ = predict_one_timed(
                model_a, image_path, device=INFER_DEVICE, imgsz=640, conf=0.3, iou=0.5, verbose=False
            )
            res_b, used_b, _ = predict_one_timed(
                model_b, image_path, device=INFER_DEVICE, imgsz=640, conf=0.3, iou=0.5, verbose=False
            )

            metrics_a = self._compare_metrics_from_result(model_a_id, res_a, used_a)
            metrics_b = self._compare_metrics_from_result(model_b_id, res_b, used_b)
            vis_a, vis_b = res_a.plot(), res_b.plot()
            self.compare_vis = self._make_compare_canvas(vis_a, metrics_a, vis_b, metrics_b)
            self.compare_metrics = {"a": metrics_a, "b": metrics_b}
            self.compare_image_path = image_path
            self.last_vis = self.compare_vis
            self._show_vis(self.compare_vis)
            self.vCompareSummary.setText(self._format_compare_summary(metrics_a, metrics_b))
            self.statusBar().showMessage("✅ 模型对比完成")
        except Exception as e:
            QMessageBox.critical(self, "❌ 模型对比失败", f"对比推理失败：{str(e)}")

    def on_save_compare(self):
        if self.compare_vis is None or self.compare_metrics is None:
            QMessageBox.information(self, "💡 提示", "暂无模型对比结果可保存")
            return
        path, _ = QFileDialog.getSaveFileName(self, "保存对比结果图片", "model_compare.jpg", "Images (*.jpg *.png)")
        if not path:
            return
        if not path.lower().endswith((".jpg", ".jpeg", ".png")):
            path += ".jpg"
        try:
            cv2.imwrite(path, self.compare_vis)
            csv_path = os.path.splitext(path)[0] + "_metrics.csv"
            a, b = self.compare_metrics["a"], self.compare_metrics["b"]
            with open(csv_path, "w", newline="", encoding="utf-8-sig") as f:
                writer = csv.writer(f)
                writer.writerow([
                    "image_name", "model_a", "model_b", "model_a_time_ms", "model_b_time_ms",
                    "model_a_fps", "model_b_fps", "model_a_object_count", "model_b_object_count",
                    "model_a_avg_conf", "model_b_avg_conf"
                ])
                writer.writerow([
                    os.path.basename(self.compare_image_path), a["model_name"], b["model_name"],
                    f"{a['time_ms']:.3f}", f"{b['time_ms']:.3f}", f"{a['fps']:.3f}", f"{b['fps']:.3f}",
                    a["object_count"], b["object_count"], f"{a['avg_conf']:.6f}", f"{b['avg_conf']:.6f}"
                ])
            self.statusBar().showMessage(f"✅ 对比图片和CSV已保存：{path}")
            QMessageBox.information(self, "✅ 保存成功", f"对比图片：\n{path}\n\n对比CSV：\n{csv_path}")
        except Exception as e:
            QMessageBox.critical(self, "❌ 保存失败", f"保存对比结果失败：{str(e)}")
    ...
    def on_open_img(self):
        ...
        res, used, wall_ms = predict_one_timed(
            model, path, device=INFER_DEVICE, imgsz=640, conf=0.3, iou=0.5, verbose=False
        )
        vis = res.plot()
        self.last_vis = vis
        self._show_vis(vis)
        ...
        self._update_stats_panel()
        self.statusBar().showMessage(f"✅ 图片检测完成：模型耗时 {used:.1f} ms，端到端 {wall_ms:.1f} ms")
    ...
    def on_open_batch(self):
        if self.batch_worker and self.batch_worker.isRunning():
            QMessageBox.information(self, "💡 提示", "批量检测正在运行，请先停止或等待完成。")
            return
        self.on_stop_video()

        input_dir = QFileDialog.getExistingDirectory(self, "选择批量图片文件夹", "")
        if not input_dir:
            return
        ...
        image_count = sum(
            ...
            and os.path.splitext(name)[1].lower() in {".jpg", ".jpeg", ".png", ".bmp"}
        )
        if image_count == 0:
            QMessageBox.information(self, "💡 提示", "所选文件夹中没有 jpg、jpeg、png 或 bmp 图片。")
            return
        ...
        output_dir = os.path.join(input_dir, f"batch_results_{time.strftime('%Y%m%d_%H%M%S')}")
        os.makedirs(output_dir, exist_ok=True)
        model_name = MODEL_SHORT_NAMES.get(self.current_model_id, self.current_model_id)
        self._reset_stats("批量图片", input_dir)
        self.batchProgress.setValue(0)
        self.batchProgress.setFormat(f"批量检测进度：0/{image_count}")

        self.batch_worker = BatchWorker(model, input_dir, output_dir, model_name, imgsz=640, conf=0.3, iou=0.5)
        self.batch_worker.frame_ready.connect(self._on_batch_frame)
        self.batch_worker.progress_ready.connect(self._on_batch_progress)
        self.batch_worker.finished.connect(self._on_batch_finished)
        self.batch_worker.error.connect(self._on_batch_error)
        self.batch_worker.start()
        self.statusBar().showMessage(f"📁 批量检测开始，共 {image_count} 张图片...")

    def _on_batch_frame(self, vis, used_ms, info, res):
        self.last_vis = vis
        self._show_vis(vis)
        self.vTime.setText(f"{used_ms:.1f}")
        self.vCount.setText(str(info["count"]))
        ...
        self.stats.update_from_result(
            res, used_ms, MODEL_SHORT_NAMES.get(self.current_model_id, self.current_model_id),
            "批量图片", frame_index=info.get("frame_index"),
            source_name=info.get("source_name", self.current_source_name)
        )
        self._update_stats_panel()

    def _on_batch_progress(self, current, total, image_name):
        percent = int(current * 100 / total) if total else 0
        self.batchProgress.setValue(percent)
        self.batchProgress.setFormat(f"批量检测进度：{current}/{total}  {image_name}")

    def _on_batch_finished(self, output_dir, processed, total, stopped):
        self.batch_worker = None
        if total:
            self.batchProgress.setValue(int(processed * 100 / total))
        report_path = os.path.join(output_dir, "detection_report.csv")
        if stopped:
            msg = f"⏸ 批量检测已停止：{processed}/{total}，报告保存在 {report_path}"
        else:
            msg = f"✅ 批量检测完成：{processed}/{total}，报告保存在 {report_path}"
        self.statusBar().showMessage(msg)
        QMessageBox.information(self, "批量检测结果", msg)

    def _on_batch_error(self, message):
        self.statusBar().showMessage(f"⚠️ 批量检测警告：{message}")
    ...
    def on_stop_batch(self):
        if self.batch_worker and self.batch_worker.isRunning():
            ...
            self.batch_worker = None
            self.statusBar().showMessage("⏸ 批量检测已停止")
    ...
    def _update_stats_panel(self):
        ...
        if hasattr(self, "labDetectionFrames"):
            if summary["source_type"] in ("图片", "批量图片"):
                self.labDetectionFrames.setText("有目标图片")
            else:
                self.labDetectionFrames.setText("有目标帧数")
    ...
    def on_clear_display(self):
        ...
        self.compare_metrics = None
        self.compare_image_path = ""
        self.vCompareSummary.setText("暂无对比结果")
        # 重置信息显示
        self.vTime.setText("-")
        self.vCount.setText("-")
        self.vMain.setText("-")
