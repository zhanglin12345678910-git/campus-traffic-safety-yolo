...
def strip_optimizer(f: Union[str, Path] = "best.pt", s: str = "", updates: dict = None) -> dict:
    ...
    for p in x_model_fp16.parameters():
        p.requires_grad = False
    ...
    if x_model_fp32:
        ...
        combined_fp32 = {**metadata, **x, **(updates or {})}
        torch.save(combined_fp32, add_suffix_to_pt_file(s or f), pickle_module=pickle)  # combine dicts (prefer to the right)
