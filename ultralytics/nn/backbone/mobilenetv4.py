...
class MobileNetV4(nn.Module):
    ...
    def forward(self, x):
        """
        前向传播，返回4个尺度的特征层（P2/4, P3/8, P4/16, P5/32）
        features: [None, None, None, None]，分别对应下采样4、8、16、32倍的特征
        """
        input_size = x.size(2)
        scale = [4, 8, 16, 32]  # 目标下采样倍数
        features = [None, None, None, None]
        for f in self.features:
            x = f(x)
            # 如果当前特征图的下采样倍数属于目标，则保存
            if input_size // x.size(2) in scale:
                features[scale.index(input_size // x.size(2))] = x
        return features  # 返回4个尺度的特征层
