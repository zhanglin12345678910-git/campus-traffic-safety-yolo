...
__all__ = ['DyHeadBlock', 'DyHeadBlockWithDCNV3', 'Fusion', 'C3k2_Faster', 'C3k2_ODConv', 'Partial_conv3', 'C3k2_Faster_EMA', 'C3k2_DBB',
           ...
           'C3k2_AdditiveBlock', 'C3k2_AdditiveBlock_CGLU', 'CSP_MSCB', 'EUCB', 'C3k2_MSMHSA_CGLU', 'C2f_SPPFEnhance', 'C2f_ASPP', 'C2f_MSConv', 'CSP_PMSFA', 'PMSFA_K333', 'CSP_PMSFA_K333', 'PMSFA_K357', 'CSP_PMSFA_K357', 'PMSFA_K579', 'CSP_PMSFA_K579', 'C3k2_MogaBlock', 'C3k2_SHSA', 'C3k2_SHSA_CGLU', 'C3k2_SMAFB', 'C3k2_SMAFB_CGLU',
           ...
           'C3k2_MambaOut_SFSC', 'C3k2_PSFSConv', 'C3k2_FasterSFSC', 'FCM', 'FCM_1', 'FCM_2', 'FCM_3', 'Pzconv', 'C3k2_GroupMamba', 'C3k2_GroupMambaBlock', 'C3k2_MambaVision'
           ]
...
class SPPFEnhanceBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        y3 = self.m(y2)
        y4 = self.m(y3)
        y = self.cv2(torch.cat((y1, y2, y3, y4), 1))
...
class ASPPBlock(nn.Module):
    def __init__(self, c, shortcut=False):
        ...
        c_ = max(c // 4, 1)
        self.cv1 = Conv(c, c_, 1, 1)
        self.cv2 = Conv(c, c_, 3, 1, d=1)
        self.cv3 = Conv(c, c_, 3, 1, d=3)
        self.cv4 = Conv(c, c_, 3, 1, d=5)
        self.cv5 = Conv(c_ * 4, c, 1, 1)
        self.add = shortcut

    def forward(self, x):
        y = self.cv5(torch.cat((self.cv1(x), self.cv2(x), self.cv3(x), self.cv4(x)), 1))
        return x + y if self.add else y
...
class MSConvBlock(nn.Module):
    def __init__(self, c, shortcut=False):
        ...
        self.cv2 = Conv(c, c_, 5, 1)
        self.cv3 = Conv(c, c_, 7, 1)
        self.cv4 = Conv(c_ * 3, c, 1, 1)
        self.add = shortcut

    def forward(self, x):
        y = self.cv4(torch.cat((self.cv1(x), self.cv2(x), self.cv3(x)), 1))
        return x + y if self.add else y


class C2f_SPPFEnhance(C2f):
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        super().__init__(c1, c2, n, shortcut, g, e)
        self.m = nn.ModuleList(SPPFEnhanceBlock(self.c, shortcut) for _ in range(n))


class C2f_ASPP(C2f):
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        super().__init__(c1, c2, n, shortcut, g, e)
        self.m = nn.ModuleList(ASPPBlock(self.c, shortcut) for _ in range(n))


class C2f_MSConv(C2f):
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        super().__init__(c1, c2, n, shortcut, g, e)
        self.m = nn.ModuleList(MSConvBlock(self.c, shortcut) for _ in range(n))

######################################## Multi-scale comparison modules end ########################################

######################################## Partial Multi-Scale Feature Aggregation Block start ########################################
...
######################################## PMSFA Ablation Variants for Kernel Scale Analysis ########################################

class PMSFA_K333(nn.Module):
    # 消融实验：卷积核尺度组合 (3, 3, 3)
    # conv1 3×3 标准卷积, conv2 3×3 逐通道卷积, conv3 3×3 逐通道卷积
    def __init__(self, inc) -> None:
        super().__init__()
...
class CSP_PMSFA_K333(C2f):
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        super().__init__(c1, c2, n, shortcut, g, e)
        self.m = nn.ModuleList(PMSFA_K333(self.c) for _ in range(n))


class PMSFA_K357(nn.Module):
    # 消融实验：卷积核尺度组合 (3, 5, 7) - 原始 PMSFA 结构
    # conv1 3×3 标准卷积, conv2 5×5 逐通道卷积, conv3 7×7 逐通道卷积
    def __init__(self, inc) -> None:
        super().__init__()
...
class CSP_PMSFA_K357(C2f):
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        super().__init__(c1, c2, n, shortcut, g, e)
        self.m = nn.ModuleList(PMSFA_K357(self.c) for _ in range(n))


class PMSFA_K579(nn.Module):
    # 消融实验：卷积核尺度组合 (5, 7, 9)
    # conv1 5×5 标准卷积, conv2 7×7 逐通道卷积, conv3 9×9 逐通道卷积
    def __init__(self, inc) -> None:
        super().__init__()
        
        self.conv1 = Conv(inc, inc, k=5)
        self.conv2 = Conv(inc // 2, inc // 2, k=7, g=inc // 2)
        self.conv3 = Conv(inc // 4, inc // 4, k=9, g=inc // 4)
        self.conv4 = Conv(inc, inc, 1)
...
class CSP_PMSFA_K579(C2f):
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        super().__init__(c1, c2, n, shortcut, g, e)
        self.m = nn.ModuleList(PMSFA_K579(self.c) for _ in range(n))
...
class C3k2_LSBlock(C3k2):
    def __init__(self, c1, c2, n=1, c3k=False, e=0.5, g=1, shortcut=True):
        super().__init__(c1, c2, n, c3k, e, g, shortcut)
        self.m = nn.ModuleList(C3k_LSBlock(self.c, self.c, n, shortcut, g) if c3k else LSBlock(self.c) for _ in range(n))
