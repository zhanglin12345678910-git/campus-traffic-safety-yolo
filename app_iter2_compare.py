...
class Main(QMainWindow):
    ...
    def on_model_compare(self):
        ...
        try:
            ...
            t0 = time.time()
            res_a = model_a.predict(image_path, device=INFER_DEVICE, imgsz=640, conf=0.3, iou=0.5, verbose=False)[0]
            used_a = (time.time() - t0) * 1000

            t0 = time.time()
            res_b = model_b.predict(image_path, device=INFER_DEVICE, imgsz=640, conf=0.3, iou=0.5, verbose=False)[0]
            used_b = (time.time() - t0) * 1000
